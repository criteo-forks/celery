            ...
            try:
                ...
                if self.always_retry and self.exception_safe_to_retry(exc):
                    if retries < self.max_retries:
                        ...
                        tb = sys.exc_info()[2]
                        raise BackendStoreError("failed to store result on the backend", task_id=task_id, state=state).with_traceback(tb)
                else:
                    raise
            ...
            try:
                ...
                if self.always_retry and self.exception_safe_to_retry(exc):
                    if retries < self.max_retries:
                        ...
                        tb = sys.exc_info()[2]
                        raise BackendGetMetaError("failed to get meta for " + task_id).with_traceback(tb)
                else:
                    raise
